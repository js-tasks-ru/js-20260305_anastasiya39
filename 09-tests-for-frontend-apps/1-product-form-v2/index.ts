...
export default class ProductForm {
  ...
  private uploadImage = async () => {
    ...
    input.onchange = async () => {
      ...
      this.sortableList?.element.append(item);
    };
  };

  private initSortableList() {
    const items = (this.productData.images || []).map((image: ProductImage) =>
      this.createImageItem(image)
    );
  ...
  private createImageItem(image: ProductImage) {
  ...
  async save(): Promise<void> {
    ...
    const images = Array.from(
      this.sortableList?.element.querySelectorAll('li') || []
    ).map((li) => {
      const url = (li.querySelector('[name="url"]') as HTMLInputElement).value;
      const source = (li.querySelector('[name="source"]') as HTMLInputElement).value;
